...
async def test_create_self_signed_app_certificate() -> None:
    ...
    cert: x509.Certificate = generate_self_signed_app_certificate(
        key, f"myserver@{hostname}", names, subject_alt_names, extended=extended, days=days_valid
    )
    ...
    assert (
        (dt_before_generation + timedelta(days_valid))
        <= cert.not_valid_after_utc
        <= (dt_after_generation + timedelta(days_valid))
    )
    ...
    ).value.digest == key_identifier_from_public_key(key.public_key())

    # check Authority Key Identifier
    auth_key_identifier: x509.AuthorityKeyIdentifier = cert.extensions.get_extension_for_class(
    ...
    assert (
        cert.extensions.get_extension_for_class(x509.SubjectAlternativeName).value.get_values_for_type(x509.DNSName)[0]
        == hostname
    )
    assert (
        ...
        == f"urn:{hostname}:foobar:myserver"
    )
...
async def test_app_create_certificate_signing_request() -> None:
    ...
    csr: x509.CertificateSigningRequest = generate_app_certificate_signing_request(
        key, f"myserver@{hostname}", names, subject_alt_names, extended=extended
    )
    ...
    # check subject alternative name
    assert (
        csr.extensions.get_extension_for_class(x509.SubjectAlternativeName).value.get_values_for_type(x509.DNSName)[0]
        == hostname
    )
    assert (
        csr.extensions.get_extension_for_class(x509.SubjectAlternativeName).value.get_values_for_type(
            x509.UniformResourceIdentifier
        )[0]
        == f"urn:{hostname}:foobar:myserver"
    )
...
async def test_app_sign_certificate_request() -> None:
    ...
    assert cert.extensions.get_extension_for_class(
        x509.SubjectKeyIdentifier
    ).value.digest == key_identifier_from_public_key(key_server.public_key())
