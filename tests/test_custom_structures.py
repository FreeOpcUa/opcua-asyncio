...
async def test_data_type_dict_add_dictionary(srv):
    ...
    assert await dict_node.read_rank() == -1
...
async def test_data_type_dict_create_data_type(srv):
    ...
    assert await desc_node.read() == type_name
    assert await desc_node.read_rank() == -1
...
async def test_data_type_dict_set_dict_byte_string(srv):
    ...
    case = Et.tostring(srv.test_etree.getroot(), encoding='utf-8').decode("utf-8").replace(' ', '')
    result = (await srv.srv.get_node(srv.dict_builder.dict_id).read()).decode("utf-8").replace(' ', '').replace('\n', '')
    assert result == case
...
async def test_functional_basic(srv):
    ...
    await basic_var.write(basic_msg)

    basic_result = await basic_var.read()
...
async def test_functional_advance(srv):
    ...
    await nested_var.write(nested_msg)

    basic_result = await basic_var.read()
    assert basic_result == basic_msg
    nested_result = await nested_var.read()
