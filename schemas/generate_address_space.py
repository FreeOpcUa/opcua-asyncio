    ...
    def make_common_variable_code(self, indent, obj):
        ...
        if obj.value is not None:
            if obj.valuetype == "ListOfExtensionObject":
                ...
                self.writecode(
                    indent,
                    '    Value=ua.Variant(LocalizedText("{0}"), ua.VariantType.LocalizedText),'.format(obj.value[1][1]),
                )
            else:
                if obj.valuetype.startswith("ListOf"):
                    obj.valuetype = obj.valuetype[6:]
                self.writecode(indent, f"    Value=ua.Variant({obj.value!r}, ua.VariantType.{obj.valuetype}),")
        if obj.rank:
...
def save_aspace_to_disk():
    ...
    asyncua.server.standard_address_space.standard_address_space.create_standard_address_space_Services = (
        asyncua.server.standard_address_space.standard_address_space_services.create_standard_address_space_Services
    )
