...
class HelloServer:
    ...
    async def init(self):
        ...
        await self.server.import_xml(os.path.join(self.model_filepath, "../nodeset/Machinery/Opc.Ua.Machinery.NodeSet2.xml"))
        await self.server.import_xml(os.path.join(self.model_filepath, "../nodeset/Woodworking/Opc.Ua.Woodworking.NodeSet2.xml"))

        # instantiate mandatory objects in server
        self.device = await instantiate(await self.server.nodes.objects.get_child("3:Machines"), await self.server.nodes.base_object_type.get_child("4:WwMachineType"), bname="test_Server_OPC_UA", dname=ua.LocalizedText("Planing Machine"), idx=4, instantiate_optional=False)
