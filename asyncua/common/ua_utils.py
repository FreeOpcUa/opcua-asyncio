...
from asyncua.ua.ua_binary import LowApiConfig
...
def val_to_string(val, truncate=False):
    ...
    if hasattr(val, "to_string"):
        ...
        if truncate and len(val) > 100:
            val = (
                val[:10].decode(LowApiConfig.String_Encoding, errors="surrogateescape")
                + "...."
                + val[-10:].decode(LowApiConfig.String_Encoding, errors="surrogateescape")
            )
        else:
            val = val.decode(LowApiConfig.String_Encoding, errors="surrogateescape")
...
def string_to_val(string, vtype):
    ...
    if vtype == ua.VariantType.Null:
        ...
        val = string.encode(LowApiConfig.String_Encoding, errors="surrogateescape")
